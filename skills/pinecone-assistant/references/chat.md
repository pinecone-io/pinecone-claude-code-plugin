...
1. Parse arguments. If assistant missing, run `uv run scripts/list.py --json` and use AskUserQuestion to ask the user to select.
2. If message missing, use AskUserQuestion to prompt the user for their question.
3. Execute:
   ```bash
...
**PINECONE_API_KEY not set** — `export PINECONE_API_KEY="your-key"`, then restart your agent session.
