...
1. **`PINECONE_API_KEY`** must be available. Check if it's set:
   ...
   ```
   If empty, ask the user to set it in their terminal: `export PINECONE_API_KEY="your-key"`

2. For the **Database path**: the Pinecone MCP server must be configured and available

## Step 0: Choose Your Path

Use the AskUserQuestion tool to ask the user which path they want:
...
**Offer to explore further:** Use AskUserQuestion to ask the user if they'd like to try another query to see the effect more clearly:
...
Use AskUserQuestion to ask the user if they want to try reranking.
...
Before anything else, use AskUserQuestion to ask the user if they have files to upload. Pinecone Assistant accepts `.pdf`, `.md`, `.txt`, and `.docx` files — a single file or a folder of files both work.
...
```
Restart your agent session after setting.
