...
2. If the user omits required arguments:
   - If only the index name is provided, use the `describe-index` tool to retrieve available namespaces and use AskUserQuestion to ask the user to choose.
   - If only a query is provided, use `list-indexes` to get available indexes, use AskUserQuestion to ask the user to pick one, then use `describe-index` for namespaces if needed.
...
If you get an access error, the key is likely missing. Ask the user to set it and restart their agent session:
```bash
